...
func main() {
	...
	if !*pqtls {
		...
		for _, k := range keysKEX {
			...
			if *cachedCert {
				_, _, connState, err := testConnHybrid(clientMsg, clientMsg, clientConfig, clientConfig, "client", *IPserver, strport)
				if err != nil {
			...
			for i := 0; i < *handshakes; i++ {
				timingState, _, _, err := testConnHybrid(clientMsg, clientMsg, clientConfig, clientConfig, "client", *IPserver, strport)
				if err != nil {
					log.Fatal(err)
				}
