...
var (
	...
	authAlgo   = flag.String("auth", "P256_Dilithium2", "Authentication Algorithm")
	rootCAAlgo = flag.String("root", "P521_Dilithium5", "Root CA Algorithm")
	intCAAlgo  = flag.String("int", "P521_Dilithium5", "Intermediate CA Algorithm")
...
func orderAlgorithmsMap() (KEXkeys []string, Authkeys []string) {
	...
	//or return a specific ordering (PQC-only not used here)
	outputKEX := []string{"P256_Kyber512", //"P384_Kyber768", "P521_Kyber1024",
		"P256_LightSaber_KEM", //"P384_Saber_KEM", "P521_FireSaber_KEM",
		"P256_NTRU_HPS_2048_509",
		//"P384_NTRU_HPS_2048_677",
		//"P521_NTRU_HPS_4096_821",
		//"P521_NTRU_HPS_4096_1229",
		//"P384_NTRU_HRSS_701", "P521_NTRU_HRSS_1373",
	}
	outputAuth := []string{
...
func nameToHybridSigID(name string) liboqs_sig.ID {
	sigId, prs := hsHybridAuthAlgorithms[name]

	if !prs {
		fmt.Println("Algorithm not found. Available algorithms: ")
		for name, _ := range hsHybridAuthAlgorithms {
			fmt.Println(name)
		}
		panic("ERROR: Algorithm not found")
	}
...
func createCertificate(pubkeyAlgo interface{}, signer *x509.Certificate, signerPrivKey interface{}, isCA bool, isSelfSigned bool) ([]byte, interface{}, error) {
	...
	} else if scheme, ok := pubkeyAlgo.(liboqs_sig.ID); ok {
...
func initCAs(rootCAAlgo, intCAAlgo interface{}) (*x509.Certificate, *x509.Certificate, interface{}) {
	/* ---------------------------- Root Certificate ---------------------------- */

	rootCACertBytes, rootCAPriv, err := createCertificate(rootCAAlgo, nil, nil, true, true)
	if err != nil {
		panic(err)
	}
	...
	intCACertBytes, intCAPriv, err := createCertificate(intCAAlgo, rootCACert, rootCAPriv, true, false)
	if err != nil {
		panic(err)
	}
...
func initServer(certAlgo interface{}, intCACert *x509.Certificate, intCAPriv interface{}) *tls.Config {
	hybridCert := new(tls.Certificate)
	var err error

	certBytes, certPriv, err := createCertificate(certAlgo, intCACert, intCAPriv, false, false)
	if err != nil {
		panic(err)
	}
